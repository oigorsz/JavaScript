...
function validateIfExistNewTest() {
    let values = JSON.parse(localStorage.getItem(localStorageKey) || "[]")
    let inputValue = document.getElementById("input_newtask").value
...
function newTask() {
    let input = document.getElementById("input_newtask");
    input.style.border = ""
    

    if (input.value == ""){
        input.style.border = "1px splid red"
        alert("Digite algo para inserir em sua lista")
    }

    else if (validateIfExistNewTest()) {
